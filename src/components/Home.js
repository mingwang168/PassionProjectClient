...
class Home extends React.Component {
  ...
  constructor(props) {
   ...
   //   console.log(data);
      if (data.length !=0) {
        this.setState({ showLearningBtn: true })
     //   console.log(this.state.showLearningBtn)
      }
    });
  ...
  handleChangeSchedule = (changeSchedule = true, dayNumber) => {
    ...
    if (changeSchedule) {
      ...
      scheduleForPut.userName = this.props.auth.user.username;
    ...
    return (
      <div className="wrap">
        <div className="scheduleBox">
          ...
          {(!this.state.showLearningBtn) && <p style={{color: "red"}}>Please upload a word list firstly ↓</p>}
          <WordList getWordNumber={this.getWordNumber} auth={this.props.auth} />
        ...
        {this.state.showLearningBtn && <Link to={{ pathname: '/learning', state: this.state.learningSchedule }}><button className="btn btn-lg startLearning">Start Learning</button></Link>}
      </div>
    );
  }
